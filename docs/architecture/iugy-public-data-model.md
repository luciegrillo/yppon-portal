...
`updated_at` é mantido por trigger no banco para que alterações feitas por
qualquer cliente preservem a trilha temporal mínima.
...
Constraints de banco mantêm a coerência do ciclo:

- `draft` não possui `published_at` nem `archived_at`;
